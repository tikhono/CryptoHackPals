...
pub fn sub_bytes(state: &mut Matrix4<u8>, sbox: &[[u8; 16]; 16]) {
    state.apply(|x| sbox[(x >> 4) as usize][(x << 4 >> 4) as usize]);
}

#[cfg(test)]
mod tests {
    use crate::{sub_bytes, INV_S_BOX};
    ...
    fn capture_the_flag() {
        let mut state = Matrix4::new(
            251, 64, 182, 81, 146, 168, 33, 80, 199, 159, 195, 24, 64, 80, 182, 255,
        );
